...
needed_programs() {
    ...
    then
        ...
        for pkmgr in xbps-install pacman; do
            ...
            case $pkmgr in
                xbps-install)
                    sudo xbps-install -S wget git make gcc curl patch
                    ;;
                pacman)
    ...
    for pkg in xbps-install pacman; do
        ...
        case $pkg in
            xbps-install)
                ...
                src_pkg_repo_install
                sudo xbps-install -S dmenu kitty xdotool polybar sxhkd nitrogen xwallpaper fzf font-awesome alacritty pcmanfm telegram-desktop chromium neofetch htop neovim;
                cd $HOME/.local/
    ...
    for pkg_mgr in xbps-install pacman; do 
        ...
        case $pkg_mgr in
            xbps-install)
                ...
                git clone https://github.com/1Meracle1/void-linux-herbstluftwm herbstluftwm
